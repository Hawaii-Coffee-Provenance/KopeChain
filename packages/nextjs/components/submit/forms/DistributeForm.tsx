...
const DistributeForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!region || !roastLevel || !existingMug) {
        throw new Error("Metadata is missing required distribution traits (Region, Roast Level, or Mug).");
      }
    ...
    try {
      await writeContractAsync(
        ...
        {
          onBlockConfirmation: () => {
            notification.success(`Batch ${form.batchName.trim()} was distributed on-chain.`);
            resetForm();
            setTimeout(() => {
              router.push("/explore");
            }, 500);
